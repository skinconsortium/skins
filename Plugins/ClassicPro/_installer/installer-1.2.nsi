...
;General
  ...
  ;Name and file
  Name "ClassicPro© v1.2 - Beta"
  
  ;RELEASE  
  ;OutFile "ClassicPro_1.2.exe"

  ;BETA STAGE  
  !define /date DATE "%Y-%m-%d"
  OutFile "ClassicPro_1.2_(${DATE}).exe"
...
Section "ClassicPro Engine" cproFiles
  ...
  File "${SOURCEPATH}\engine\flex\scripts\*.m"
  File "${SOURCEPATH}\engine\flex\scripts\*.maki"

  ;SetOutPath $INSTDIR\Plugins\ClassicPro\engine\one\scripts\attribs
  ;File "${SOURCEPATH}\engine\one\scripts\attribs\*.m"

  SetOutPath $INSTDIR\Plugins\ClassicPro\engine\flex\xml
  File "${SOURCEPATH}\engine\flex\xml\*.xml"

  SetOutPath $INSTDIR\System
  File "C:\Program Files\Winamp\System\ClassicProFlex.w5s"

  SetOutPath "$INSTDIR\Skins\cProFlex - iFlex"
  File /r "C:\Program Files\Winamp\Skins\cProFlex - iFlex\"
  SetOutPath "$INSTDIR\Skins\cProFlex - Xenolith"
  File /r "C:\Program Files\Winamp\Skins\cProFlex - Xenolith\"
