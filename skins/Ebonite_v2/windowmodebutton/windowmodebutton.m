...
Global layout n, s, m, v, mv, cn;
Global int nX, nY, sX, sY, mX, mY, vX, vY, mvX, mvY, iMode, cnX, cnY;
...
System.onScriptLoaded() 
{
	...
	menu_Layouts.addCommand("Ebonite Window Modes", 0, 0, 1);
...
bGotContainer = 0;
	if (strsearch(strlower(lclick), "switchto:") >= 0) {
		...
		if (c) 
		{
			...
			if(gotLayoutString=="narrow")
			{
				cn = c.getLayout(gotLayoutString);
				resizeLayout(cn);
			}
...
resizeLayout(layout newlayout)
{
  if (bGotContainer == -1)
  {
	...
	if(gotLayoutString=="narrow")
	{
		cnX = getPrivateInt(getSkinName(), "cn_X", 0);
		cnY = getPrivateInt(getSkinName(), "cn_Y", 0);
		newlayout.resize(cnX, cnY, newlayout.getWidth(), newlayout.getHeight());
		return;
	}
...
cn.onEndMove() {
  if (cn.isVisible()) {
    setPrivateInt(getSkinName(), "cn_X", cn.getLeft());
    setPrivateInt(getSkinName(), "cn_Y", cn.getTop());
  }
}
...
//this may well be borking peoples keyboard shortcuts, leave out for now
