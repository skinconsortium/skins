...
notifier_timer.onTimer() {
	...
	if (notifier_fdout_alpha.getData() == "1") {
		...
		if (notifier_layout.getGuiY() == 2) sy = -NOTIFIER_H;
		else sy = geth + NOTIFIER_H;
...
showNotifier(int w) {
	...
	if (notifier_fdin_alpha.getData() == "1") {
		if (!notifier_layout.isVisible()) notifier_layout.resize(x, y, w, NOTIFIER_H);
		else
		{
			notifier_layout.resize(notifier_layout.getguiX(), y, notifier_layout.getGuiW(), NOTIFIER_H);
		}
		if (notifier_layout.isTransparencySafe()) {
			...
			notifier_layout.setTargetH(NOTIFIER_H);
			notifier_layout.setTargetSpeed(StringToInteger(notifier_fadeintime_attrib.getData()) / 1000);
			notifier_layout.gotoTarget();
		} else {
		...
		if (y == 2) sy = -NOTIFIER_H;
		else sy = geth + NOTIFIER_H;
		if (!notifier_layout.isVisible()) notifier_layout.resize(x, sy, w, NOTIFIER_H);
		else
		{
			notifier_layout.resize(notifier_layout.getguiX(), y, notifier_layout.getGuiW(), NOTIFIER_H);
		}
		...
		if (!notifier_layout.isVisible()) notifier_layout.resize(sx, y, w, NOTIFIER_H);
			else
		{
			notifier_layout.resize(notifier_layout.getguiX(), y, notifier_layout.getGuiW(), NOTIFIER_H);
		}
...
Int fillNextTrackInfo(String corneroverride) {
	...
	if (plentry)
	{ 
		plentry.setText(integerToString(getPlaylistIndex()+1)+" of "+integerToString(getPlaylistLength()));
		plentry.show();
		s_plentry.setText(integerToString(getPlaylistIndex()+1)+" of "+integerToString(getPlaylistLength()));
		s_plentry.show();
	}
	if (nexttrack) {
		if (corneroverride == "") {
	...
	if (album) { 
		...
		else
		{
			...
			else
			{
				...
				s_album.setText("");
				s_album.setXmlParam("display", "songinfo");				
			}
		}
		album.show(); 
		s_album.show(); 
	...
	if (showAlbumArt)
	{
		...
		if (g_text) g_text.setXmlParam("x", "93");
		if (g_text) g_text.setXmlParam("w", "-110");		
	}
	else
	{
	...
	if (maxv < _title.getAutoWidth()) maxv = _title.getAutoWidth()+5; //within reason
	...
	//within reason chop it 1/4 screen is way to drastic
	if (maxv > getw/2) maxv = getw/2;
	
	
	return maxv + ( showAlbumArt * 77 ) + 1; // Adds 60 extra pixels if album art is visible
}
...
Int fillCustomInfo(String customtext)
{
	...
	if (endofplayback != NULL && s_endofplayback  != NULL) {
		...
		s_endofplayback.setText(customtext);
