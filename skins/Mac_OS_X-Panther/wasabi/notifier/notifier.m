...
System.onScriptLoaded() {
	...
	show_not_fade = _configimp.findObject("Transition").findObject("show"); //!
*/
	endTimer = new Timer;
...
/*	Group _left;
	...
	if (notifier_aa_enabled.getData() == "1") {
		if (notifier_aa_right.getData() == "1") {
			layer c_status = _right.findObject("cover.status");
			if (c_status.isVisible() && cover_web_attrib.getData() == "0") { w = w + 32; _norm.show(); }
			else { _right.show(); w = w + 32 + 68; }
		} if (notifier_aa_left.getData() == "1") {
			layer c_status = _left.findObject("cover.status");
			if (c_status.isVisible() && cover_web_attrib.getData() == "0") { w = w + 32; _norm.show(); }
...
/*	Group _left;
	...
	if (notifier_aa_enabled.getData() == "1") {
		...
		layer c_status = p.findObject("cover.status");
		if (c_status.isVisible() && cover_web_attrib.getData() == "0") p = _norm;
	} else p = _norm;
