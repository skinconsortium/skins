...
#define NOTAV "CoverSearch via Amazon.com isn't aviable!"
#define AV "CoverSearch via Amazon.com is aviable!"
#define WRONGPATH "Invalid Path - CoverSearch via Amazon.com isn't aviable!"
...
Global Button btn_download, btn_apply;
...
System.onScriptLoaded() {
	...
	if (getParam() == "1") {
		...
		e_pixpath.setText(getPrivateString("Deimos/CoverSearch/", "pixPath", ""));
		...
		e_htmlpath.setText(getPrivateString("Deimos/CoverSearch/", "amazon.com_htmlpath", ""));

		btn_download = XUIGroup.findObject("download");
		btn_apply = XUIGroup.findObject("apply");
		Dummy = XUIGroup.findObject("cover.dummy");
		statustxt = XUIGroup.findObject("statustxt");
		if (getPrivateString("Deimos/CoverSearch/", "amazon.com_htmlpath", "") == "") {
			statustxt.setText(NOTAV);
		} else statustxt.setText(AV);
...
e_pixpath.onEditUpdate() {
	setPublicString("Deimos/CoverSearch/_pixPath", getText());
}
...
btn_download.onLeftClick() {
   	navigateUrl("http://home.arcor.de/martin.deimos/downloads/DpW-addons/DpW-addon__Mac_OS_X_-_Panther__CoverSearch.exe");
}

btn_apply.onLeftClick() {
	boolean b = FileSearch(e_htmlpath.getText(), "160noimage.jpg");
	if (b) {
		statustxt.setText(AV);
		setPublicString("Deimos/CoverSearch/_amazon.com_htmlpath", e_htmlpath.getText());
	} else {
		statustxt.setText(WRONGPATH);
		messageBox("Your folder doesn't contain the CoverSearch files!\nPlease check it again!" , "Error", 1, "");
		setPublicString("Deimos/CoverSearch/_amazon.com_htmlpath", "");
	}
}
