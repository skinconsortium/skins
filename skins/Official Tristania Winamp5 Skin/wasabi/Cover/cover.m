...
tmr.onTimer() {
	if (count == 3) {
		tmr.stop();
		int i_answer = messageBox("You haven't configured the CoverSearch via Amazon.com!\nWeb search is your best Cover search option for this skin!\nWill you do this now?" , "Please Configure CoverSearch via Amazon.com", 12, "");
		if (i_answer == 4) {
			prefs_visible_attrib.setData("1");
			Container c = System.getContainer("config");
			if (c) { layout l =  c.getLayout("normal");
				if (l) {
					l.setFocus();
					l.center();
					GuiTree _tree = l.findObject("wasabi.preferences.group").findObject("wasabi.preferences.tree.embedded").findObject("mylist");
					_tree.sendAction("switchToItem", "Amazon.com Config", 0, 0, 0, 0);
				}
			}
		}
