// by Potashnikoff Aleksey aka NickMikh
// Script for use in OneDirectionText Object
// Version: 1.01.02

// !!!Always use the same script and xml file version!!!

#include <lib/std.mi>
#include <lib/config.mi>

#define TEXT_PROPS "antialias,Font,fontsize,color,shadowcolor,shadowx,shadowy,timeroffstyle,timecolonwidth,forcefixed,forceupcase,forceuppercase,forcelocase,forcelowercase,bold,wrap,valign,offsety,dblclickaction,rightclickaction,move,tooltip"

Function chooseBehavior(int UserWidth, int UserHeight, boolean UserDeclare);
...
System.onScriptLoaded()
{
	...
	strBetween = "***";
	intTextDirect = 1;
	txtMain = SG.getObject("My.Text.Main");
	txt2 = SG.getObject("My.Text.2");
	moveText = new Timer;
}

SG.OnSetVisible(boolean onoff)
{
	choosebehavior(0,0,0);	
}
System.onSetXuiParam(String param, String value)
{
	...
	if (strlower(param) == "direction")
	{
		intTextDirect = StringtoInteger(Value);
	}
	if (strlower(param) == "ttip")
	{
		txtMain.setXMLParam("tooltip",Value);
	}
...
chooseBehavior(int UserWidth,int UserHeight, boolean UserDeclare)
{
	int intTextHeight;
	moving = 0;
	TextWidth = SG.GetWidth();
	intTextHeight = SG.GetHeight();
	If (UserDeclare)
	{
		TextWidth = UserWidth;
		intTextHeight = UserHeight;
	}
	AutoWidth = txtMain.getAutoWidth();
	int AutoWidth2;
	If (AutoWidth <= TextWidth || NoSpeed || TextWidth <= 0 || intTextHeight <= 0 || !SG.IsVisible())
	{
		txtMain.Show();
		txt2.Hide();
		moveText.Stop();
...
CopyTextProperties()
{
	for (Int i = 0; i <= 20; i++)
	{
		If (txtMain.getXMLParam(System.getToken(TEXT_PROPS,",",i)) != txt2.getXMLParam(System.getToken(TEXT_PROPS,",",i)))
		{
