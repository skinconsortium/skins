...
System.onScriptLoaded()
{
	...
	Close = frame_layout.findObject("Close");
	...
	//does this fix non dta on startup?
//	delay.start();
//	sync.start();
...
System.onScriptUnloading()
{
	...
	//frame_cont.close();		// CUT, does this fix the crash bug?! It seems so
	frame_cont = NULL;
	comp_layout = NULL;
	scriptGroup = NULL;
}
...
System.onSetXuiParam(String param, String value)
{
	...
	{
		windowdisableresize.setXMLParam("visible", "0");
	}
}
...
comp_layout.onSetVisible (Boolean on)
{
	if (on)
	{
		if (!comp_layout.getContainer().isDynamic()) 
		{
			system.onScriptLoaded();
		}
...
sync.onTimer ()
{
	...
	if (comp_layout.getAlpha() != frame_layout.getAlpha())
	{
		...
		comp_layout.setAlpha(1);
		frame_layout.setAlpha(1);
		frame_alpha = frame_layout.getAlpha();
		comp_alpha = comp_layout.getAlpha();
...
CopyProperties()
{
	for (Int i = 0; i <= 19; i++)
	{
	comptitle.setText(integertostring(i));
...
frame_layout.onResize (int x, int y, int w, int h)
{
	r_bypass = false;
	frame_layout.setfocus();
	comp_layout.setfocus();
...
desktopalpha_enabled_attrib.onDataChanged ()
{
	...
	{
		desktopalpha_enabled_attrib.setData("1");
	}/*
