...
Cover_visible_attrib.onDataChanged() {
	if (mychange) return;
	mychange = 1;
	if (getData() == "1") System.getContainer("cover").show();
	if (getData() == "0") System.getContainer("cover").hide();
	mychange = 0;
}
...
System.onKeyDown(String key) {
  if (key == "alt+c") {
    if (cover_visible_attrib.getData() == "0") {
        cover_visible_attrib.setData("1");
    } else {
        cover_visible_attrib.setData("0");
    } complete;
  }
  if (key == "alt+p") {
