...
/*	l_Dummy.setXMLParam("image", "/../" + str_skin + "/skinfo.png");
	...
	if (i_len > 0) {
		...
		map m;
		m = new map;
		m.loadmap(str_skinp + "skinfo.png");
		int i_len = m.getheight();
		delete m;
		if (i_len == 0 || i_len == 64 || str_skinp == "") {
			l_info.setXMLParam("image", "clear");
			t_Skinfo.setText("No info aviable for this skin!");
