 ...
 * songinfo.m
 *
 * Handles general fileinfo stuff
 *
...
Global String layerID;

System.onScriptLoaded()
{
	group PlayerDisplay = getScriptgroup();

	layerID = PlayerDisplay.getParentLayout().getID();
...
System.onStop ()
{
	monster.setXmlParam("image", "player."+layerID+".indicator.out.na");
	songInfoTimer.stop();
}
...
delayload.onTimer ()
{
	delayload.stop();
	int c = getChannels();
	if (c == -1) monster.setXmlParam("image", "player."+layerID+".indicator.out.na");
	else if (c == 1) monster.setXmlParam("image", "player."+layerID+".indicator.out.mono");
	else if (c == 2 || c == 3) monster.setXmlParam("image", "player."+layerID+".indicator.out.stereo");
	else monster.setXmlParam("image", "player."+layerID+".indicator.out.surround");
